...
\item[mtraction:]  magnetic traction information [optional]
...
\item[step:]      frequency/time step information
\item[save:]      options to save data
\item[sealevel:]  ocean information for sea-level simulation
\item[ice:]       ice information for sea-level simulations
...
\begin{adescription}{nl\_maxiter}
  \item[utm\_zone]  The UTM (Universal Transverse Mercator) zone [integer, optional, valid range (1, 60)]. This is required for regional simulations.
  ...
  \item[pot\_type] : potential type [string, optional,\texttt{\sq{gravity}} = Gravity potential, \texttt{\sq{magnetic}} = Magnetic potential, \texttt{\sq{electric}} = Electrical potential due to a current, \texttt{\sq{charge}} = Electrical potential due to a charge density]. It must be provided if \texttt{pot\_dof = 1}.
  \item[grav0]  : switch to activate background gravity (Cowling approximation) [integer, optional, 0 = OFF, 1 = ON, default $\Rightarrow$ 0].
  \\
\end{adescription}
...
\begin{adescription}{nl\_maxiter}
  ...
  \item[fsfile]: file name of free surface [string]. Use an empty file if there is no free surface or the free surface isn't necessary.
...
\begin{adescription}{  add\_infmesh}
  ...
  \item[if infbc = 1, i.e., infinite BC]
...
\begin{adescription}{nl\_maxiter}
  ...
  \item[if type = 0: Slip source]
  \item[  slipfile]: file name of slip information [string].
  \item[if type = 1: CMTSOLUTION]
  \item[  cmtfile]: file name of CMT information [string].
  \item[  mapto]: file name of CMT information [string, optional, \texttt{\sq{GLOBE}} = iGlobal spherical coordinates, \texttt{\sq{UTM}} = UTM coordinates, \texttt{\sq{none}} = None, default $\Rightarrow$ \texttt{\sq{none}}].
  \item[if type = 2: Finite fault]
  \item[  faultfile]: file name for the fault information [string].
  \item[  faultmetafile]: file name for the fault meta file [string].
  \item[if type = 1: Slip with split node]
...
\begin{adescription}{nl\_maxiter}
  ...
  \item[plastic]: flag to implement plasticity [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \\
\end{adescription}
...
\texttt{\underline{step:}}
\begin{adescription}{  start}
  \item[type]: step type [integer, 0 = Time, 1 = Frequency, default $\Rightarrow$ 0].
  \item[step]: step interval [real].
  \item[if type = 1, i.e., frequency]
  \item[  start]: start frequency [real].
  \item[  end]: end frequency [real].
  \item[  funit]: frequency unit [string, optional, `Hz'].
  \item[if type = 0, i.e., time]
  \item[  nstep]: number of time steps [integer > 0, default $\Rightarrow$ 1].
  \item[  tunit]: time unit [string, optional, `sec', `hr',`day', `yr', default $\Rightarrow$ `sec'].
  \\
\end{adescription}
...
\texttt{\underline{station:}}
\begin{adescription}{sfile}
  \item[sfile]: file name of stations.
  \\
\end{adescription}

\texttt{\underline{save:}}
\begin{adescription}{pstress}
  \item[disp]: displacement field [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[stress]: stress field [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[strain]: strain field [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[pstress]: principal stress field [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[model]: model properties [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[gpot]: gravitational potential [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[mpot]: magnetic potential [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[epot]: electric potential [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[agrav]: gradient of potential, e.g., acceleration due to gravity [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[magb]: magnetic field, B [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \\
\end{adescription}


\texttt{\underline{sealevel:}}
\begin{adescription}{solvesl}
  \item[slfile]: sea level file name  [string].
  \item[savesl0]: saves initial sea level [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[saveOF]: saves the ocean function [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \item[solvesl]: solve for sea level? [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \\
\end{adescription}
...
\texttt{\underline{ice:}}
\begin{adescription}{solvesl}
  \item[icefile]: ice file name  [string].
  \item[saveice0]: saves initial ice distribution [integer, optional, 0 = No, 1 = Yes, default $\Rightarrow$ 0].
  \\
\end{adescription}
...
\emph{BC\_type val}\\
     $\Rightarrow$\emph{BC\_type} (integer, 0=nodal, 1=edge, 2=face)\\
...
\subsection{Fault slip files: \texttt{faultslipfile\_plus, faultslipfile\_minus}}
...
\subsection{Magnetic traction file: \texttt{trfile}}
...
\emph{traction type} (integer, 0 = inherit magnetization from the parent elements, 1 = define uniform magnetization)\\
if \emph{traction type} = 0\\
  Nothing to do\\
if \emph{traction type} = 1\\
  \emph{Magnetization, FlagForInclinationOrLatitude (0=inc or 1=lat),InclinationOrLatitudeAngle Declination, Azimuth} (SI unit but angle in degrees)\\
\emph{number of entities} (points for point load or faces for distributed load)\\
\emph{elementID entityID \\
elementID entityID \\
elementID entityID \\
...
\begin{itemize}
 \item The \emph{materialID} must be in a sequential order starting from 1.
 \item The \emph{doaminID} represents the material domain (e.g., 1 = elastic or 11 = viscoelastic).
 \item The \emph{type} represents the type of material properties input.
 \begin{itemize}
  \item \emph{type}=0: Homogeneous material block $\Rightarrow$ define $\gamma$, $E$, $\nu$, $\phi$, $c$, and $\psi$.
  \item \emph{type}=-1: Tomographic model input defined on the structured grid $\Rightarrow$ define file name, \emph{file}.
  \end{itemize}

  \item The $\gamma$ represents the unit weight in N/m\tsup{3}.
  \item The $E$ the Young's modulus of elasticity in N/m\tsup{2}.
  \item The $\phi$ the angle of internal friction in degrees.
  \item The $c$ the cohesion in N/m\tsup{2}.
  \item The $\psi$ the angle of dilation in degrees.
  \item The \emph{file} is the file name of the tomographic structured grid input.
\end{itemize}
...
\\
\texttt{magnetization:}\\
\emph{NumberOfMagnetizationBlocks\\
materialID, Magnetization, FlagForInclinationOrLatitude (0=inc or 1=lat), InclinationOrLatitudeAngle, Declination, Azimuth} (SI unit but angle in degrees)\\
...\\
...\\
\\
\\
For electrical potential due to electric current, add the following information in the material list file:\\
\\
\texttt{electrical\_conductivty:}\\
\emph{NumberOfElectricalConductivityBlocks\\
materialID, materialType, Conductivity1, Conductivity2, Conductivity3, $\alpha$, $\beta$, $\gamma$}\\
...\\
...\\
\\

\emph{materialType}=0: isotropic and \emph{materialType}=1: anisotropic. For isotropic material, only \emph{Conductivity1} is relevant.
\\
For electrical potential due to electric charge density, add the following information in the material list file:\\
\\
\texttt{charge\_density:}\\
\emph{NumberOfChargeDensityBlocks\\
materialID, Charge\_Density}\\
...\\
...\\
...
\subsection{Sea Level file: \texttt{slfile}}

This file contains details required for running a sea level simulation.
In this file any lines beginning with a hashtag as the first character are considered
to be comment lines and are ignored. The information format for this file (ignoring any comment lines) is: 


\begin{adescription}{slfile format:}
\item[line 1:]        \emph{cartesian or global}: integer value 1 = Cartesian, 2 = Global
\item[line 2:]        \emph{type of initial SL}: integer value 0 = defined by constant Z value - only for cartesian SL simulation  
\item[line 3:]        \emph{initial sea level} See full description below \\ 
\end{adescription}



\subsubsection{initial sea level (line 3):}
Different formats are used based on the input: 

if line 2 = 0 (Sea level is defined by constant Z value). In this case line 3 is the Z value for a cartesian simulation.


\subsection{Ice file: \texttt{icefile}}

This file contains details for ice parameters when running a sea level simulation.
In this file any lines beginning with a hashtag as the first character are considered
to be comment lines and are ignored. \\ 

The user can add in as many ice 'objects' as they wish. Objects are defined by a type value and specific parameters. The maximum number of parameters is currently 5, and varies for each type of object. Objects can be: 

\begin{table}[h!]
\resizebox{\textwidth}{!}{%
\begin{tabular}{llll}
\hline
\multicolumn{1}{|c|}{\textbf{Object type}} & \multicolumn{1}{c|}{\textbf{Typeval}} & \multicolumn{1}{l|}{\textbf{Parameters}} & \multicolumn{1}{l|}{\textbf{Description}} \\ \hline
Delta (single point) & 1 & node  height & Adds ice of a given height (above the solid surface) to a specific node in the mesh \\
Vertical cylinder & 2 & x, y, radius, height & Vertical cylinder of given height centred on x,y location
\end{tabular}%
}
\end{table}
...
\begin{adescription} {}
\item[line 1:]        \emph{number of ice objects to add} 
\item[line 2-:]       \emph{object1}  typeval params  
\end{adescription}


Example:\\
Here we specify that there will be 2 single bits of ice, placed at nodes 410 and 414 with heights of 5 and 7 respectively, as well as and a cylinder placed at location (x = 4.5, y = 5.5) on the solid surface with a radius of 3.14 and a height of 10. \\\\
\texttt{
3\\
1 414 7 \\
2 4.5 5.5 3.14 10\\
