...
\texttt{git clone \-\-recursive https://github.com/homnath/SPECFEMX.git}\\

Alternatively, the original \pack\ package comes in a single compressed file \linebreak\texttt{\pack.tar.gz}, which can be extracted using \texttt{tar} command:\\
...
\begin{itemize}[-]
  ...
  \item \underline{A recent FORTRAN compiler}. The software is written mainly in FORTRAN 90, but it also uses a few FORTRAN 2008 features (e.g., streaming IO). These features are already available in most of the FORTRAN compilers, e.g., gfortran version $\ge$ 4.2 (\texttt{gcc.gnu.org/wiki/GFortran}) and g95 (\texttt{www.g95.org}).
\end{itemize}
...
\item[ENABLE\_MPI]                    : If \texttt{ON}, the main parallel program \texttt{pspecfemx} is built otherwise main serial program \texttt{specfemx} is built. The default is \texttt{OFF}.
\item[ENABLE\_PETSC]                    : If \texttt{ON}, the program will be compiled using the \texttt{PETSc} libraries. The default is \texttt{OFF}.
...
\item To run the serial program, type \\
    \texttt{./bin/specfemx} \emph{input\_file\_name}

    Example:

    \texttt{./bin/specfemx ./input/validation1.sem}
...
\item To run the parallel program, type \\
    \texttt{mpirun -n} \emph{number\_of\_nodes} \texttt{./bin/pspecfemx} \emph{input\_file\_name} \\

    OR

    \texttt{mpirun -n} \emph{number\_of\_nodes} \texttt{-{}-hostfile} \emph{host\_file} \texttt{./bin/pspecfemx} \emph{input\_file\_name}

    Example:

    \texttt{mpirun -n 8 ./bin/pspecfemx ./input/validation1.psem}
\end{itemize}
