...
! bring theta between 0 and PI, and phi between 0 and 2*PI
  ...
  use math_constants, only: ZERO,PI,TWOPI,ZEROTOL
  ...
  if (abs(theta) < ZEROTOL ) theta = theta + 0.0000001d0
  if (abs(phi) < ZEROTOL ) phi = phi + 0.0000001d0
  ...
  if (ph < ZERO .or. ph > TWOPI) then
    i = abs(int(ph/TWOPI))
    if (ph < ZERO) then
      ph = ph+(i+1)*TWOPI
    else
      if (ph > TWOPI) ph=ph-i*TWOPI
    endif
    phi=ph
  endif
  ...
  if (phi < ZERO .or. phi > TWOPI) stop 'phi out of range in reduce'
...
! convert x y z to r theta phi, single precision call
  ...
  use math_constants, only: SMALL_VAL_ANGLE,ZERO
  ...
  theta = real(datan2(dsqrt(xmesh*xmesh+ymesh*ymesh),zmesh), kind=kreal)

  if (xmesh > -SMALL_VAL_ANGLE .and. xmesh <= ZERO) xmesh = -SMALL_VAL_ANGLE
  if (xmesh < SMALL_VAL_ANGLE .and. xmesh >= ZERO) xmesh = SMALL_VAL_ANGLE

  phi = real(datan2(ymesh,xmesh), kind=kreal)

  r = real(dsqrt(xmesh*xmesh + ymesh*ymesh + zmesh*zmesh), kind=kreal)
...
! converts geocentric coordinates x/y/z to geographic radius/latitude/longitude (in degrees)
  ...
  use math_constants, only: HALFPI
  use conversion_constants, only: RAD2DEG
  ...
  lat = (HALFPI - theta_prime) * RAD2DEG
  lon = phi * RAD2DEG
...
! gives some thoughts.
  ...
  use math_constants, only: HALFPI,ZEROTOL
  use earth_constants, only: ASSUME_PERFECT_SPHERE,USE_OLD_VERSION_5_1_5_FORMAT,ONE_MINUS_F_SQUARED
  ...
  if (.not. ASSUME_PERFECT_SPHERE) then
    ...
    if (USE_OLD_VERSION_5_1_5_FORMAT) then
      theta_prime = HALFPI - datan(1.006760466d0*dcos(theta)/dmax1(ZEROTOL,dsin(theta)))
    else
      ! converts geocentric colatitude theta to geographic colatitude theta_prime
      theta_prime = HALFPI - datan(FACTOR_TAN*dcos(theta)/dmax1(ZEROTOL,dsin(theta)))
    endif
  else
...
! converts geocentric colatitude (theta) to geographic colatitude (theta_prime) (in radians)
  ...
  real(kind=kreal),intent(out) :: theta_prime
  ...
  theta_prime = real(dtheta_prime, kind=kreal)
...
! converts geographic latitude (lat_prime) (in degrees) to geocentric colatitude (theta) (in radians)
  ...
  use earth_constants, only: ASSUME_PERFECT_SPHERE,ONE_MINUS_F_SQUARED
  ...
  if (.not. ASSUME_PERFECT_SPHERE) then
    ...
    theta = HALFPI - atan( ONE_MINUS_F_SQUARED*dtan(lat_prime * DEG2RAD) )
  else
    ...
    theta = HALFPI - lat_prime * DEG2RAD
  endif
...
! latitude in degree between [-90,90], longitude in degree between [0,360]
  ...
  use math_constants, only: INFTOL
  ...
  real(kind=kreal), dimension(nglob),intent(in) :: xstore,ystore,zstore
