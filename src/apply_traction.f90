  ...
  traction: do
    ...
    elseif(tractype==23)then ! torsion
     ...
     trac_stat=.true.
      ...
      write(errtag,'(a,i0,a)')'ERROR: traction type ',tractype,' not supported!'
      exit traction
    endif
  enddo traction
  close(11)
...
!      load(igdof)=load(igdof)+ufs0(i_node,i_dof)
!    enddo
!  enddo
!  
!  deallocate(ufs0)
...
if(isfstraction0.and.nnode_fs>0)then
  ...
  do i_face=1,nelmt_fs
    ...
    do i_gll=1,nfgll
      ...
      q=ufs0(ifnode,:) ! vector of nodal values

      igdofu=gdof(idofu,numf(i_gll))
