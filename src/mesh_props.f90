...
!real(kind=kreal), dimension(NGLOB_AB),intent(in) :: xstore,ystore,zstore
!
!logical, dimension(NSPEC_AB) :: ispec_is_acoustic,ispec_is_elastic,ispec_is_poroelastic
!
!real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(in) :: kappastore,mustore,rhostore
!real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(in) :: phistore,tortstore
!real(kind=kreal), dimension(2,NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(in) :: rhoarraystore
!real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(in) :: rho_vpI,rho_vpII,rho_vsI
!
!double precision,intent(in) :: DT
!real(kind=kreal),intent(inout) :: model_speed_max,min_resolved_period
!
!! local parameters
!real(kind=kreal) :: vpmin,vpmax,vsmin,vsmax,vpmin_glob,vpmax_glob,vsmin_glob,vsmax_glob
!real(kind=kreal) :: vp2min,vp2max,vp2min_glob,vp2max_glob
!
!real(kind=kreal) :: poissonmin,poissonmax,poissonmin_glob,poissonmax_glob
!real(kind=kreal) :: distance_min,distance_max,distance_min_glob,distance_max_glob
!real(kind=kreal) :: elemsize_min,elemsize_max,elemsize_min_glob,elemsize_max_glob
!real(kind=kreal) :: x_min,x_max,x_min_glob,x_max_glob
!real(kind=kreal) :: y_min,y_max,y_min_glob,y_max_glob
!real(kind=kreal) :: z_min,z_max,z_min_glob,z_max_glob
!real(kind=kreal) :: cmax,cmax_glob,pmax,pmax_glob
!real(kind=kreal) :: dt_suggested,dt_suggested_glob,avg_distance
!real(kind=kreal) :: vel_min,vel_max
!
!logical:: DT_PRESENT
!
...
!real(kind=kreal),dimension(1) :: tmp_val
!
!! debug: for vtk output
!real(kind=kreal),dimension(:),allocatable :: tmp1,tmp2
!integer :: ier
...
!! checks mesh
!if (myrank == 0) then
!  if (distance_min_glob <= 0.0_kreal) then
...
!  endif
!  if (elemsize_min_glob <= 0.0_kreal) then
...
!  real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: rho_vpI,rho_vpII,rho_vsI
!  real(kind=kreal), dimension(2,NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: rhoarraystore
!  real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: phistore,tortstore
!  real(kind=kreal), dimension(NGLOB_AB) :: xstore,ystore,zstore
!  integer, dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: ibool
!  double precision :: DT
!  real(kind=kreal) :: model_speed_max,min_resolved_period
!
...
!implicit none
!
!real(kind=kreal),intent(out) :: vpmin,vpmax,vsmin,vsmax,poissonmin,poissonmax
!
!integer :: ispec
!logical :: has_vs_zero
!
!integer :: NSPEC_AB
!real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: kappastore,mustore,rhostore
!
!! local parameters
!real(kind=kreal) :: vp_sq,vs_sq,poisson
!integer :: i,j,k
...
!          poisson = 0.5_kreal * (vp_sq - 2.0_CUSTOM_REAL * vs_sq) / (vp_sq - vs_sq)
...
!  implicit none
!
!  real(kind=kreal) :: vpmin,vpmax,vsmin,vsmax
!
!  integer :: ispec
!  logical :: has_vs_zero
!
!  integer :: NSPEC_AB
!  real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: &
!    kappastore,mustore,rho_vp,rho_vs
!
!  ! local parameters
!  real(kind=kreal), dimension(NGLLX,NGLLY,NGLLZ):: vp_elem,vs_elem
!  real(kind=kreal), dimension(1) :: val_min,val_max
!
!  ! initializes
!  vpmin = HUGEVAL
!  vpmax = -HUGEVAL
...
!implicit none
!
!real(kind=kreal) :: vpmin,vpmax,vp2min,vp2max,vsmin,vsmax
!
!integer :: ispec
!logical :: has_vs_zero
!logical :: has_vp2_zero
...
!
!! local parameters
!real(kind=kreal) :: vp,vp2,vs,tort,rho_s,rho_f,phi
!integer :: i,j,k
!integer :: incrx,incry,incrz
!
!! element looping
...
!        vs   = rho_vsI(i,j,k,ispec) / ( (1.0_kreal - phi) * rho_s + phi * rho_f - phi/tort * rho_f )
!        vp  = rho_vpI(i,j,k,ispec) / ( (1.0_kreal - phi) * rho_s + phi * rho_f - phi/tort * rho_f )
!        vp2 = rho_vpII(i,j,k,ispec)/ ( (1.0_kreal - phi) * rho_s + phi * rho_f - phi/tort * rho_f )
!      else
!        vs   = rho_vsI(i,j,k,ispec) / ( (1.0_kreal - phi) * rho_s + phi * rho_f )
!        vp  = rho_vpI(i,j,k,ispec) / ( (1.0_kreal - phi) * rho_s + phi * rho_f )
!        vp2 = rho_vpII(i,j,k,ispec)/ ( (1.0_kreal - phi) * rho_s + phi * rho_f )
!      endif
!
!      ! statistics
!      ! vp
...
use math_constants, only: HUGEVAL
...
real(kind=kreal),intent(out) :: distance_min_glob,distance_max_glob
real(kind=kreal),intent(out) :: elemsize_min_glob,elemsize_max_glob
real(kind=kreal),intent(out) :: x_min_glob,x_max_glob
real(kind=kreal),intent(out) :: y_min_glob,y_max_glob
real(kind=kreal),intent(out) :: z_min_glob,z_max_glob

! local parameters
real(kind=kreal) :: distance_min,distance_max
real(kind=kreal) :: elemsize_min,elemsize_max
real(kind=kreal) :: x_min,x_max
real(kind=kreal) :: y_min,y_max
real(kind=kreal) :: z_min,z_max
integer :: ispec
...
! we purposely do not include the distance along the diagonals of the element,
! only along its three coordinate axes.
...
real(kind=kreal) :: x1,x2,y1,y2,z1,z2
